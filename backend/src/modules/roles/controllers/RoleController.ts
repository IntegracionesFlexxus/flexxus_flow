...
export class RoleController {
  ...
  /**
   * GET /api/v1/roles/company
   * Obtener roles de la empresa del usuario autenticado
   */
  ...
  async getAllPermissions(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      console.log('🎯 [RoleController] getAllPermissions called');
      console.log('🔍 [RoleController] roleService exists:', !!this.roleService);
      ...
      console.log('📋 [RoleController] Permissions result:', {
      ...
      // Asegurar que siempre retornamos datos válidos
      ...
      console.error('❌ [RoleController] Error in getAllPermissions:', error);
      next(error);
    }
  }
  ...
  async validateName(req: AuthRequest, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      const isValid = await this.roleService.validateRoleName(
        name,
        targetCompanyId,
        excludeId
      );
